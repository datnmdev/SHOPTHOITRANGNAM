$(document).ready(function () {
    $('#pagination-previous').click(function (e) { 
        ...
        location.assign('/owner/suppliers?page-number=' + (currentPageNumber - 1 == 0 ? pageCount : currentPageNumber - 1))        
    });
    ...
    $('#pagination-next').click(function (e) {
        ...
        var currentPageNumber = Number.parseInt($('#pagination').attr('current-page-number'))
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign('/owner/suppliers?page-number=' + (currentPageNumber + 1 > pageCount ? 1 : currentPageNumber + 1))        
    });
    ...
    fetch("/owner/suppliers?export", {method: "GET"})
        .then(response => response.json())
        .then(data => {
            var convertedSupplierData = [["Mã nhà cung cấp", "Tên nhà cung cấp", "Số điện thoại", "Email", "Tình trạng"]]
            data.forEach(supplierData => {
                let supplier = []
                supplier.push(supplierData.supplierCode)
                supplier.push(supplierData.supplierName)
                supplier.push(supplierData.phoneNumber)
                supplier.push(supplierData.email)
                supplier.push(supplierData.contractStatus ? 'Còn hợp đồng' : 'Hết hợp đồng')

                convertedSupplierData.push(supplier)
            })

            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet('Danh sách nhà cung cấp');
            worksheet.addRows(convertedSupplierData);
            workbook.xlsx.writeBuffer().then(buffer => {
                const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const url = URL.createObjectURL(blob);
                $('#export').attr('href', url);
                $('#export').attr('download', 'danh_sách_nhà_cung_cấp.xlsx')
                $('#export').click()
            })
        })
    ...
    $("form#goto-page-form").submit(function(e) {
        ...
        location.assign("/owner/suppliers?page-number=" + pageNumber)
    })

    $("#first-page-btn").click(function() {
        location.assign("/owner/suppliers?page-number=" + 1)
    })

    $("#last-page-btn").click(function() {
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign("/owner/suppliers?page-number=" + (isNaN(pageCount) ? 1 : pageCount))
    })
