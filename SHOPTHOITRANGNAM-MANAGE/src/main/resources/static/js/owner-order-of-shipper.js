$(document).ready(function () {
    var deliveryStaffCode = $('#delivery-staff-code').val();
    ...
    fetch(`/owner/orders?delivery-staff-code=${deliveryStaffCode}&export`, {method: "GET"})
        ...
        .then(data => {
            ...
            const worksheet = workbook.addWorksheet('Danh sách đơn hàng của shipper');
            worksheet.addRows(convertedOrderData);
            workbook.xlsx.writeBuffer().then(buffer => {
                const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const url = URL.createObjectURL(blob);
                $('#export').attr('href', url);
                $('#export').attr('download', 'danh_sách_đơn_hàng_của_shipper.xlsx')
                $('#export').click()
            })
        })
