$(document).ready(function () {
    $('#pagination-previous').click(function (e) { 
        ...
        location.assign('/owner/accounts?page-number=' + (currentPageNumber - 1 == 0 ? pageCount : currentPageNumber - 1))        
    });
    ...
    $('#pagination-next').click(function (e) {
        ...
        var currentPageNumber = Number.parseInt($('#pagination').attr('current-page-number'))
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign('/owner/accounts?page-number=' + (currentPageNumber + 1 > pageCount ? 1 : currentPageNumber + 1))        
    });
    ...
    fetch("/owner/accounts?export", {method: "GET"})
        .then(response => response.json())
        .then(data => {
            var convertedAccountData = [["Tên đăng nhập", "Thời điểm tạo", "Đã kích hoạt", "Trạng thái", "Loại tài khoản"]]
            data.forEach(accountData => {
                let account = []
                account.push(accountData.username)
                account.push(accountData.creationTime)
                account.push(accountData.accountActivation)
                account.push(accountData.status ? 'Đang hoạt động' : 'Đã bị khóa')
                account.push(accountData.accountType.accountTypeName)

                convertedAccountData.push(account)
            })

            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet('Danh sách tài khoản');
            worksheet.addRows(convertedAccountData);
            workbook.xlsx.writeBuffer().then(buffer => {
                const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const url = URL.createObjectURL(blob);
                $('#export').attr('href', url);
                $('#export').attr('download', 'danh_sách_tài khoản.xlsx')
                $('#export').click()
            })
        })
    ...
    $("form#goto-page-form").submit(function(e) {
        ...
        location.assign("/owner/accounts?page-number=" + pageNumber)
    })

    $("#first-page-btn").click(function() {
        location.assign("/owner/accounts?page-number=" + 1)
    })

    $("#last-page-btn").click(function() {
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign("/owner/accounts?page-number=" + (isNaN(pageCount) ? 1 : pageCount))
    })
