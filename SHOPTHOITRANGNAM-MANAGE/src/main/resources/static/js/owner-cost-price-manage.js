$(document).ready(function () {
    var supplyDetailId = $('#supply-detail-id').val()
    ...
    $('#pagination-previous').click(function (e) { 
        ...
        location.assign(`/owner/cost-prices?supply-detail-id=${supplyDetailId}&page-number=` + (currentPageNumber - 1 == 0 ? pageCount : currentPageNumber - 1))        
    });
    ...
    $('#pagination-next').click(function (e) {
        ...
        var currentPageNumber = Number.parseInt($('#pagination').attr('current-page-number'))
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign(`/owner/cost-prices?supply-detail-id=${supplyDetailId}&page-number=` + (currentPageNumber + 1 > pageCount ? 1 : currentPageNumber + 1))        
    });
    ...
    fetch(`/owner/cost-prices?supply-detail-id=${supplyDetailId}&export`, {method: "GET"})
        .then(response => response.json())
        .then(data => {
            var convertedCostPriceData = [["ID", "Giá nhập", "Thời điểm áp dụng", "IDCTCC"]]
            data.forEach(costPriceData => {
                let costPrice = []
                costPrice.push(costPriceData.costPriceId)
                costPrice.push(costPriceData.price)
                costPrice.push(costPriceData.effectiveTime)
                costPrice.push(costPriceData.supplyDetailId)

                convertedCostPriceData.push(costPrice)
            })

            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet('Lịch sử cập nhật giá nhập sản phẩm');
            worksheet.addRows(convertedCostPriceData);
            workbook.xlsx.writeBuffer().then(buffer => {
                const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const url = URL.createObjectURL(blob);
                $('#export').attr('href', url);
                $('#export').attr('download', 'lịch_sử_cập_giá_nhập_sản_phẩm.xlsx')
                $('#export').click()
            })
        })
    ...
    $("form#goto-page-form").submit(function(e) {
        ...
        location.assign(`/owner/cost-prices?supply-detail-id=${supplyDetailId}&page-number=` + pageNumber)
    })

    $("#first-page-btn").click(function() {
        location.assign(`/owner/cost-prices?supply-detail-id=${supplyDetailId}&page-number=` + 1)
    })

    $("#last-page-btn").click(function() {
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign(`/owner/cost-prices?supply-detail-id=${supplyDetailId}&page-number=` + (isNaN(pageCount) ? 1 : pageCount))
    })
