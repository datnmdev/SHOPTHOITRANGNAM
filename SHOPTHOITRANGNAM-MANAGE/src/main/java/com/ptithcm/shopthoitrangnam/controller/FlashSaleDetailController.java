...
public class FlashSaleDetailController {
	...
	public String createFlashSaleDetailPage(Model model, @RequestParam("flash-sale-id") Integer flashSaleId) {
		...
		List<ProductDetail> productDetailsInFlashSaleDetails = new ArrayList<>(flashSaleDetailService.findByFlashSale(flashSaleService.findByFlashSaleId(flashSaleId).get()).stream().map(flashSaleDetail -> flashSaleDetail.getProductDetail()).toList());
	...
	public String createFlashSaleDetail(@Valid @ModelAttribute(name = "flashSaleDetailDto") FlashSaleDetailDto flashSaleDetailDto, BindingResult bindingResult, Model model) {
		...
		List<ProductDetail> productDetailsInFlashSaleDetails = new ArrayList<>(flashSaleDetailService.findByFlashSale(flashSaleService.findByFlashSaleId(flashSaleDetailDto.getFlashSaleId()).get()).stream().map(flashSaleDetail -> flashSaleDetail.getProductDetail()).toList());
		List<ProductDetail> productDetails = new ArrayList<>(productDetailService.findAll());
		productDetails.removeAll(productDetailsInFlashSaleDetails);
		model.addAttribute("productDetails", productDetails);
