...
public class EmployeeController {
	...
	public String employeePage(Model model, RedirectAttributes redirectAttributes) {
		...
		Integer quantityOfWarehouseStaff = employeeService.findAll().stream().filter(employee -> employee.getPosition().getPositionCode().equals(Role.WAREHOUSE_WORKER.getCode())).toList().size();
		Integer quantityOfShipper = employeeService.findAll().stream().filter(employee -> employee.getPosition().getPositionCode().equals(Role.SHIPPER.getCode())).toList().size();
		model.addAttribute("quantityOfWarehouseStaff", quantityOfWarehouseStaff);
		model.addAttribute("quantityOfShipper", quantityOfShipper);
	...
	@GetMapping(value = "/owner/employees", params = "position-code")
	public String filterByStatusOrder(Model model, @RequestParam("position-code") String positionCode) {
		Integer quantityOfWarehouseStaff = employeeService.findAll().stream().filter(employee -> employee.getPosition().getPositionCode().equals(Role.WAREHOUSE_WORKER.getCode())).toList().size();
		Integer quantityOfShipper = employeeService.findAll().stream().filter(employee -> employee.getPosition().getPositionCode().equals(Role.SHIPPER.getCode())).toList().size();
		model.addAttribute("quantityOfWarehouseStaff", quantityOfWarehouseStaff);
		model.addAttribute("quantityOfShipper", quantityOfShipper);
		
		List<Employee> employees = employeeService.findAll().stream().filter(employee -> employee.getPosition().getPositionCode().equals(positionCode)).toList();
		model.addAttribute("employees", employees);
		model.addAttribute("pageNumber", 0);
		return "owner-employee-manage.html";
	}
