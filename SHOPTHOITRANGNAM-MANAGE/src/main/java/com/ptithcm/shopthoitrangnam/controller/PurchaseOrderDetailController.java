...
public class PurchaseOrderDetailController {
	...
	public String createPurchaseOrderDetailPage(Model model, @RequestParam("purchase-order-code") String purchaseOrderCode) {
		...
		List<ProductDetail> productDetailsInPurchaseOrderDetails = new ArrayList<>(purchaseOrderDetailService.findByPurchaseOrder(purchaseOrderService.findByPurchaseOrderCode(purchaseOrderCode).get()).stream().map(purchaseOrderDetail -> purchaseOrderDetail.getProductDetail()).toList());
		List<ProductDetail> productDetails = new ArrayList<>(purchaseOrderService.findByPurchaseOrderCode(purchaseOrderCode).get().getSupplier().getSupplyDetails().stream().map(supplyDetail -> supplyDetail.getProductDetail()).toList());
	...
	public String createPurchaseOrderDetail(@Valid @ModelAttribute(name = "purchaseOrderDetailDto") PurchaseOrderDetailDto purchaseOrderDetailDto, BindingResult bindingResult, Model model) {
		...
		List<ProductDetail> productDetailsInPurchaseOrderDetails = new ArrayList<>(purchaseOrderDetailService.findByPurchaseOrder(purchaseOrderService.findByPurchaseOrderCode(purchaseOrderDetailDto.getPurchaseOrderCode()).get()).stream().map(purchaseOrderDetail -> purchaseOrderDetail.getProductDetail()).toList());
		List<ProductDetail> productDetails = new ArrayList<>(purchaseOrderService.findByPurchaseOrderCode(purchaseOrderDetailDto.getPurchaseOrderCode()).get().getSupplier().getSupplyDetails().stream().map(supplyDetail -> supplyDetail.getProductDetail()).toList());
		productDetails.removeAll(productDetailsInPurchaseOrderDetails);
		model.addAttribute("productDetails", productDetails);
