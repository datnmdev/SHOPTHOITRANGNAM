$(document).ready(function () {
    $('#pagination-previous').click(function (e) { 
        ...
        location.assign('/owner/positions?page-number=' + (currentPageNumber - 1 == 0 ? pageCount : currentPageNumber - 1))        
    });
    ...
    $('#pagination-next').click(function (e) {
        ...
        var currentPageNumber = Number.parseInt($('#pagination').attr('current-page-number'))
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign('/owner/positions?page-number=' + (currentPageNumber + 1 > pageCount ? 1 : currentPageNumber + 1))        
    });
    ...
    fetch("/owner/positions?export", {method: "GET"})
        .then(response => response.json())
        .then(data => {
            var convertedPositionData = [["Mã chức vụ", "Tên chức vụ", "Mô tả"]]
            data.forEach(positionData => {
                let position = []
                position.push(positionData.positionCode)
                position.push(positionData.positionName)
                position.push(positionData.description)

                convertedPositionData.push(position)
            })

            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet('Danh sách chức vụ');
            worksheet.addRows(convertedPositionData);
            workbook.xlsx.writeBuffer().then(buffer => {
                const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const url = URL.createObjectURL(blob);
                $('#export').attr('href', url);
                $('#export').attr('download', 'danh_sách_chức_vụ.xlsx')
                $('#export').click()
            })
        })
    ...
    $("form#goto-page-form").submit(function(e) {
        ...
        location.assign("/owner/positions?page-number=" + pageNumber)
    })

    $("#first-page-btn").click(function() {
        location.assign("/owner/positions?page-number=" + 1)
    })

    $("#last-page-btn").click(function() {
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign("/owner/positions?page-number=" + (isNaN(pageCount) ? 1 : pageCount))
    })
