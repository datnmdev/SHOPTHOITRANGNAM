$(document).ready(function () {
    $('#pagination-previous').click(function (e) { 
        ...
        location.assign('/owner/employees?page-number=' + (currentPageNumber - 1 == 0 ? pageCount : currentPageNumber - 1))        
    });
    ...
    $('#pagination-next').click(function (e) {
        ...
        var currentPageNumber = Number.parseInt($('#pagination').attr('current-page-number'))
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign('/owner/employees?page-number=' + (currentPageNumber + 1 > pageCount ? 1 : currentPageNumber + 1))        
    });
    ...
    fetch("/owner/employees?export", {method: "GET"})
        .then(response => response.json())
        .then(data => {
            var convertedEmployeeData = [["Mã nhân viên", "Họ", "Tên", "Ngày sinh", "Giới tính", "Số điện thoại", "email", "Ngày vào làm", "Tình trạng", "Chức vụ", "Tên đăng nhập", "Hình ảnh"]]
            data.forEach(employeeData => {
                let employee = []
                employee.push(employeeData.employeeCode)
                employee.push(employeeData.firstName)
                employee.push(employeeData.lastName)
                employee.push(employeeData.dateOfBirth)
                employee.push(employeeData.gender ? 'Nữ' : 'Nam')
                employee.push(employeeData.phoneNumber)
                employee.push(employeeData.email)
                employee.push(employeeData.startDate)
                employee.push(employeeData.jobStatus ? 'Đang làm việc' : 'Đã nghỉ việc')
                employee.push(employeeData.position)
                employee.push(employeeData.username)
                employee.push(employeeData.image)

                convertedEmployeeData.push(employee)
            })

            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet('Danh sách nhân viên');
            worksheet.addRows(convertedEmployeeData);
            workbook.xlsx.writeBuffer().then(buffer => {
                const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
                const url = URL.createObjectURL(blob);
                $('#export').attr('href', url);
                $('#export').attr('download', 'danh_sách_nhân_viên.xlsx')
                $('#export').click()
            })
        })
    ...
    $("form#goto-page-form").submit(function(e) {
        ...
        location.assign("/owner/employees?page-number=" + pageNumber)
    })

    $("#first-page-btn").click(function() {
        location.assign("/owner/employees?page-number=" + 1)
    })

    $("#last-page-btn").click(function() {
        var pageCount = Number.parseInt($('#pagination').attr('page-count'))
        location.assign("/owner/employees?page-number=" + (isNaN(pageCount) ? 1 : pageCount))
    })
