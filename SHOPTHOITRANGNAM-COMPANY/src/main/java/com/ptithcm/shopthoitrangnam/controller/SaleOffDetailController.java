...
public class SaleOffDetailController {
	...
	public String createSaleOffDetailPage(Model model, @RequestParam("sale-off-id") Integer saleOffId) {
		...
		List<ProductDetail> productDetailsInSaleOffDetails = saleOffDetailService.findBySaleOff(saleOffService.findBySaleOffId(saleOffId).get()).stream().map(saleOffDetail -> saleOffDetail.getProductDetail()).toList();
	...
	public String createSaleOffDetail(@Valid @ModelAttribute(name = "saleOffDetailDto") SaleOffDetailDto saleOffDetailDto, BindingResult bindingResult, Model model) {
		...
		List<ProductDetail> productDetailsInSaleOffDetails = saleOffDetailService.findBySaleOff(saleOffService.findBySaleOffId(saleOffDetailDto.getSaleOffId()).get()).stream().map(saleOffDetail -> saleOffDetail.getProductDetail()).toList();
		List<ProductDetail> productDetails = productDetailService.findAll();
		productDetails.removeAll(productDetailsInSaleOffDetails);
		model.addAttribute("productDetails", productDetails);
